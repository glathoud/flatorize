        ...
        for(j=0;j<J;++j) {
            ...
            for(i=I-1;i!==-1;--i) {
                if(i!==j) {
                    ...
                    for(k=J-1;k>0;--k) { Ci[k] -= Cj[k]*x; --k; Ci[k] -= Cj[k]*x; }
                    if(k===0) Ci[0] -= Cj[0]*x;
                }
            }
        }
...
function hand_direct_fm_inv_of_ij( /*integer*/I, /*?integer?*/J )
{
    ...
    return hand_direct_fm_inv;

    function hand_direct_fm_inv
...
(function fm_speedtest_unittest() {
    ...
    , obtained = hand_direct_fm_inv_of_ij( 4 )( m, c )
    , expected = c
    ;
    obtained === expected  ||  null.bug;
    1e-10 > c.reduce(
