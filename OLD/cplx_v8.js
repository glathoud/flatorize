...
//
// Diff with ./cplx_v7.js: much faster (CPU) and thinner (RAM) stat
// gathering based on much better expression instance unity.  Now
// `dft512flat` and `dft1024flat` can be generated.
// 
// Guillaume Lathoud
// January 2013
//
...
// circa 0.08 second in V8 3.7.12 on Ubuntu 12.04
// circa      second in Chrome 23 on Ubuntu 12.04
// circa      second in Firefox 17 on Ubuntu 12.04
// (V8) dft64flat code length: 33229
time('dft64flat');
var dft64flat = directF('arr', dft_exprgenF( 6 ));
timeEnd('dft64flat');
log('dft64flat code length: ' + (''+dft64flat).length);

// circa 0.36 second  in V8 3.7.12 on Ubuntu 12.04
// circa      seconds in Chrome 23 on Ubuntu 12.04
// circa      seconds in Firefox 17 on Ubuntu 12.04
// (V8) dft128flat code length: 84727
time('dft128flat');
var dft128flat = directF('arr', dft_exprgenF( 7 ));
timeEnd('dft128flat');
log('dft128flat code length: ' + (''+dft128flat).length);    

// circa 2.0  seconds in V8 3.7.12  on Ubuntu 12.04
// circa      seconds in Chrome 23  on Ubuntu 12.04
// circa      seconds in Firefox 17 on Ubuntu 12.04
// (V8) dft256flat code length: 203401
time('dft256flat');
...
// circa 11.8 seconds in V8 3.7.12  on Ubuntu 12.04
// circa      seconds in Chrome 23  on Ubuntu 12.04
// circa      seconds in Firefox 17 on Ubuntu 12.04
// (V8) dft512flat code length: 480726
if (false)
{
    time('dft512flat');
...
// circa 72.7 seconds in V8 3.7.12  on Ubuntu 12.04
// circa      seconds in Chrome 23  on Ubuntu 12.04
// circa      seconds in Firefox 17 on Ubuntu 12.04
// (V8) dft1024flat code length: 1126204
if (false)
{
    time('dft1024flat');
...
function expr()
{
    ...
    // Update the cache
    ret.__exprIdnum__ = exprCache.count++;
    idstr2expr[ idstr ] = ret;
...
function swiF(/*comma-separated string*/varstr, /*function*/exprgen)
{
    ...
    function switcher_getDirect()
    {
        if (!direct)
        {
            ...
            swiF.exprCache = { idstr2expr: {}, count: 0 };
...
// one walk to generate the code including temporary variable names.
{
    ...
    {
        if (gathering && ('__exprIdnum__' in code))
        {
            // During the first pass: statistics gathering.  Do not
            // care about generating valid code, but rather try to
            // generate useful statistics without exploding the CPU
            // load/RAM consumption.
            // 
            // This special case was necessary to be able to generate
            // code at all for the use cases dft512flat and
            // dft1024flat (not working yet in ./cplx_v7.js). It also
            // led to much faster overall code generation for all the
            // other cases.
            
            // First, depth-first walk to recursively count the stats of the expression's parts,
            expr2str( code, cfg );         

            // then return a shortcut for faster stat_gathering (less CPU AND less memory) - invalid code, does not matter.
            ret = '#' + code.__exprIdnum__.toString(36); 
            
            // Notes:
            //
            // While in theory the above two code lines can lead to
            // less optimal solutions than using the actual full, raw,
            // valid expression string, this issue is counterbalanced
            // by the fact that we already strive for expression
            // instance unity (see `expr()` function: `if (idstr in
            // idstr2expr)`).
        }
        else
        {
            // During the second pass: code generation
    ...
    else if (shorthand = cfg  &&  'object' === typeof code  &&  ('__gather_strid__' in code)  &&  cfg.duplistrid2shorthand[ code.__gather_strid__ ])
        ret = shorthand;
...
{
    if (cfg.isTop)
        return code2stat( code, cfg = { strcount: 0
                                        , str2strid: {}
                                        , strid2count: {}, strid_pile: [], strid2inpile: {}, strid2expr: {}, strid2varname: {}
                                        , varname: Object.create( cfg.varname )
    ...
    var duplicates = cfg.strid_pile.filter( function (strid) { return cfg.strid2count[ strid ] > 1; } );
    
    var duplistrid2shorthand = cfg.duplistrid2shorthand = {};
    for (var i = duplicates.length; i--;)
    {
        var strid = duplicates[ i ];
        cfg.duplistrid2shorthand[ strid ] = cfg.strid2varname[ strid ];
    }
    ...
    cfg.varInitArr = duplicates.map( function (strid) { 
        
        var tmp = Object.create( duplistrid2shorthand );
        tmp[ strid ] = null;

        var tmpcfg = { duplistrid2shorthand: tmp };
        tmpcfg[ CODE2STR_CFG_ID ] = cfgid;
        
        return cfg.strid2varname[ strid ] + ' = ' + code2str( cfg.strid2expr[ strid ], tmpcfg, true );
    } );
    ...
    function stat_gathering( code, /*string*/str )
    {
        if (isExpr( code ))
        {
            var strid;
            if ('__gather_strid__' in code)
            {
                strid = code.__gather_strid__;
            }
            else if (str in cfg.str2strid)
            {
                strid = cfg.str2strid[ str ];
            }
            else
            {
                var strid = cfg.str2strid[ str ] = cfg.strcount++; // `strid`: integer
                cfg.strid2count[ strid ] = 0;
                ...
                cfg.strid2varname[ strid ] = varname;
            }

            if (!('__gather_strid__' in code))
                code.__gather_strid__ = strid;
            
            
            cfg.strid2count[ strid ]++;

            if (!(strid in cfg.strid2inpile))
            {
                cfg.strid_pile.push( strid );
                cfg.strid2inpile[ strid ] = 1;
            }
            cfg.strid2expr[ strid ] = code;
        }
    }
}
