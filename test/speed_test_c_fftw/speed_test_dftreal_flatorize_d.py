...
import json, os, pprint, re, shutil, stat, sys
sys.path.append( '..' )
...
def speed_test_dftreal_flatorize_d( dftsize, verbose = True ):
    ...
    for one in ({ PREFIX : 'flatorize_d_64bit_', PRECISION : PRECISION_DOUBLE, },
        ...
        tmp = extless + '_speed_test.gdc'
        filename_speed_test_gdc_sh  = { False : tmp + '32.sh',  True : tmp + '64.sh' }
        filename_speed_test_gdc_bin = { False : tmp + '32.bin', True : tmp + '64.bin' }

        tmp = extless + '_speed_test.ldc2'
        filename_speed_test_ldc2_sh  = { False : tmp + '32.sh',  True : tmp + '64.sh' }
        filename_speed_test_ldc2_bin = { False : tmp + '32.bin', True : tmp + '64.bin' }
        ...
        fcf_li = ((filename_speed_test_dmd_sh,  'dmd',  filename_speed_test_dmd_bin, ),
                  (filename_speed_test_gdc_sh,  'gdc',  filename_speed_test_gdc_bin, ),
                  (filename_speed_test_ldc2_sh, 'ldc2', filename_speed_test_ldc2_bin,),
                  )

        for fn_sh_map,compilname,fn_bin_map in fcf_li:
            ...
            open( fn_sh, 'wb' ).write( test_compile_sh_code( info, pathless_from( filename_h ), pathless_from( filename_d ), pathless_from( filename_speed_test_d ), compilname = compilname, bits64 = bits64 ).encode( ENCODING )  )
