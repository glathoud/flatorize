...
CUSTOM_INIT_D_CODE = 'custom_init_d_code'
...
SRCDIR = 'test_d.srcdir'
OUTDIR = 'test_d.outdir'
...
def copy_src( srcdir, outdir, verbose_prefix=None ):
    ...
    for filename in glob.glob( os.path.join( srcdir, '*.d' )):
...
def test_d( verbose=True ):
    ...
    if verbose:
        print( '' )
        print( INDENT + 'test_d: 0. setup outdir: ' + outdir )

    ensure_dir( outdir, empty = True )

    copy_src( srcdir, outdir, verbose_prefix = verbose and (INDENT + 'test_d: 0. ') )

    #

    if verbose:
        print()
        print( INDENT + 'test_d: 1. pass all asm.js tests, 2. get their configuration and generate D code...' )
    ...
    jscode = ' '.join( [ 'load(\'asmjs/tests.js\');'
                         ...
                         , 'if (!flatorize.getCodeD) load( \'flatorize_d.js\' );'
                         , 'var output = {};'
    ...
    if verbose:
        ...
        print()
        print( INDENT + 'test_d: 3. write out .h, .d and .sh files for each example...' )
    ...
    if verbose:
        print()
        print( '...done with `test_d`: {0}'.format( summary( out_arr )[ MESSAGE ] ) )
...
def assert_test( name, info, outdir, verbose ):
    ...
    filename_h        = filename_base + '_decl.d'
    filename_d        = filename_base + '.d'
    filename_test_d   = filename_base + '_test.d'

    extless = os.path.splitext( filename_test_d )[ 0 ]

    filename_test_dmd_sh  = extless + '.dmd.sh'
    filename_test_dmd_bin = extless + '.dmd.bin'

    pathless_test_dmd_bin = os.path.split( filename_test_dmd_bin )[ 1 ]
    ...
    open( fn_sh, 'wb' ).write(
        test_compile_sh_code( 
            info, filename_h, filename_d, filename_test_d, 
            ).encode( ENCODING ) 
        )
    ...
    fn_test_sh  = filename_test_dmd_sh
    fn_test_bin = filename_test_dmd_bin
...
int main( int argc, char **argv )
{
  ...
  ''' + simple_input_init_d_code( info ) + (
      '' if not info[ HAS_ARRAY ] else os.linesep.join( map( lambda s: INDENT + s, infostruct_init_d_code( info )))
      ) + (
      (os.linesep + info[ CUSTOM_INIT_D_CODE ])  if  CUSTOM_INIT_D_CODE in info  else ('''
...
''' + ('' if not info[ HAS_ARRAY ] else os.linesep.join( map( lambda s: INDENT + s, infostruct_done_d_code( info )))) + '''
...
def array_expected_output_name( info ):
    ...
    if info[ HAS_ARRAY ] and not (CUSTOM_INIT_D_CODE in info):
...
def arrayinit_d_code( info, expected_output_alone = False, one_name = None ):
    ...
    c_open = comment_d_code( '[' )
    c_close = comment_d_code( ']' )
    ...
    for x in array_test_sorted:
        ...
        if is_output and not single:
            lines.append( comment_d_code( ( 'output'  if  is_output  else  'input' ) + ' array "' + x[ NAME ] + '"' + ( ' (initialization to 0)'  if  (is_output and not single)  else  '') ) )
            lines.append( '' )
...
def simple_input_init_d_code( info ):

    lines = []

    if CUSTOM_INIT_D_CODE not in info:
...
def infostruct_init_d_code( info ):
...
def infostruct_done_d_code( info ):
...
def scalar_expected_output_init_d_code( info ):
...
def unit_test_output_d_code( info ):
...
def test_compile_sh_code( info, filename_h, filename_d, filename_test_d, bits64=True ):
    ...
    filename_test_bin = os.path.splitext( filename_test_d )[ 0 ] + dot + 'bin'
...
def special_test_scalarint_from_scalardouble_forbidden( out_arr, verbose = True ):
    ...
    jscode = '''load('flatorize.js'); load('flatorize_d.js');
    ...
    try {
        ...
        , plusone_asmjs_gen    = flatorize.getCodeD( { switcher : plusone, name : plusone_asmjs_name } )
        ;
    } catch (e) {
