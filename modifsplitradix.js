...
function dft_msr_genF( radix, /*?object?*/opt )
...
//
// Naive implementation without any flatorizing.
//
// Options: `opt = { real : true | false (default), hermihalf : true | false (default), true only makes sense when real==true as well }`
{
    var      real = opt && opt.real
    ...
    function exprfun(/*array of float | array of [ float, float ]*/arr)
    {
        return hermihalf
            ? newfft( Ntop, arr, 0, 1).slice( 0, 1 + (Ntop >> 1) )
            : newfft( Ntop, arr, 0, 1 )
        ;
    }

    function newfft( N, arr, shift, mul )
    // Returns an array of code expressions, one for each point of the
    // DFT output (array length: 2^radix).
    {
        var ret;
        ...
        else
        {
            ...
            ,  u  = newfft ( N_2, arr, shift,       mul * 2 )
            ,  z  = newfftS( N_4, arr, shift + mul, mul * 4 )
            ,  zp = newfftS( N_4, arr, shift - mul, mul * 4 )
            ;
            ret = new Array( N );
            for (var k = 0; k < N_4; k++)
            {
    ...
    function newfftS( N, arr, shift, mul )
    // Computes DFT / S_{N,k}
    {
        var ret;
        if (N === 1)
        ...
        else
        {
            ...
            ,  u  = newfftS2( N_2, arr, shift,       mul * 2 )
            ,  z  = newfftS ( N_4, arr, shift + mul, mul * 4 )
            ,  zp = newfftS ( N_4, arr, shift - mul, mul * 4 )
            ;
            ret = new Array( N );
    ...
    function newfftS2( N, arr, shift, mul )
    // Computes DFT / S_{2N,k}
    {
        ...
        else
        {
            ...
            ,  u  = newfftS4( N_2, arr, shift,       mul * 2 )
            ,  z  = newfftS ( N_4, arr, shift + mul, mul * 4 )
            ,  zp = newfftS ( N_4, arr, shift - mul, mul * 4 )
            ;
            ret = new Array( N );
    ...
    function newfftS4( N, arr, shift, mul )
