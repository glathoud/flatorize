...
(function () {
    ...
    // }}}
    {
        ...
        {
            return helper_c_code( fixed2, opt );
        }
    }
    ...
    {
        var lines = [ 
            line_comment_code( '--- Code generated by helper_h_code' + ( !opt  ?  ''  :  '(opt: ' + JSON.stringify( opt  ||  null ) + ')' ) + ' ---' ) 
            , ''
        ]
        ...
        if (arr  &&  arr.length)
        {
            ...
            arr.forEach( function ( name ) { push_array_info_constants( an2i[ name ] ); } );


            lines.push( '' 
                        , line_comment_code( 'Helper functions to allocate/free ' + sca_name + ' and give convenient pointers' )
                        , helper_init_decl( fixed, opt )
                        , helper_done_decl( fixed, opt ) 
                      );
            ...
            lines.push.apply(
                ...
                ( 
                    [ 
                        line_comment_code( 'convenient pointers to parts of `' + sca_name + '`'  )
                    ].concat( 
        ...
        lines.push( '' , line_comment_code( 'Generated implementation' ), funDeclCodeC( fixed ) + ';' );

        
        
        return lines.join( '\n' );

        function push_array_info_constants( info )
        {
            var NAME = info.name.toUpperCase();
            lines.push( 
                'const int FZC_' + NAME + '_N = ' + info.n + ';' 
            );
        }
    }
    
    function helper_c_code( /*object*/fixed, /*?object?*/opt )
    {
        var lines = [ line_comment_code( 'Code generated by helper_c_code' + ( !opt  ?  ''  :  '(opt: ' + JSON.stringify( opt  ||  null ) + ')' ) ) ];
    ...
    function helper_init( /*object*/fixed)
    {
    ...
    function helper_done_decl( /*object*/fixed, /*?object?*/opt )
    {
        ...
        return 'void ' + new Array( STRUCT_NAME.length-2 ).join( ' ' ) + fixed.topFunName + '_mfree( ' + STRUCT_NAME + '* );';
    }
